<script lang="ts">
  ...
  import { fetchArchivedDirections, restoreDirection, type Direction } from '../../stores/directions'

  const back = () => navigate('directions')
  let archived: Direction[] = []

  async function load(): Promise<void> {
    archived = await fetchArchivedDirections()
  }

  async function onRestore(id: number): Promise<void> {
    const ok = await restoreDirection(id)
    if (ok) archived = archived.filter(d => d.id !== id)
  }
...
<div class="max-w-md w-full px-6 py-10">
  ...
  <div class="flex items-center justify-between gap-3 mb-4">
    <h2 class="text-2xl font-semibold">{$t('directions')} — {$t('archive')}</h2>
  </div>

  <div class="mt-4 grid grid-cols-1 gap-3">
    {#each archived as dir}
      <div class="w-full rounded-lg bg-slate-100 px-2 py-2 ring-1 ring-black/5 dark:bg-slate-800/60 dark:ring-white/10 shadow-sm flex items-center gap-2">
        ...
        <button class="rounded-md bg-emerald-600/90 hover:bg-emerald-600 active:bg-emerald-700 px-3 py-2 text-xs font-medium transition text-white" on:click={() => onRestore(dir.id)}>
          {$t('restore')}
        </button>
      </div>
    {/each}
