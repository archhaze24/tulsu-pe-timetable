...
<div class="max-w-md w-full px-6 py-10">
  ...
  <div class="flex items-center justify-between gap-3">
    ...
    <button class="rounded-md bg-slate-100 hover:bg-slate-200 active:bg-slate-300 px-3 py-2 text-xs font-medium shadow-sm ring-1 ring-black/5 transition dark:bg-slate-800/60 dark:hover:bg-slate-800 dark:active:bg-slate-700 dark:ring-white/10" on:click={() => navigate('directions_archive')}>
      {$t('archive')}
    </button>
  </div>
  <div class="mt-6 grid grid-cols-1 gap-3">
    {#each $directionsStore as dir}
      <div class="w-full rounded-lg bg-slate-100 px-2 py-2 ring-1 ring-black/5 dark:bg-slate-800/60 dark:ring-white/10 shadow-sm flex items-center gap-2">
        <button class="flex-1 text-left rounded-md hover:bg-slate-200 active:bg-slate-300 dark:hover:bg-slate-800 dark:active:bg-slate-700 px-3 py-2 transition" on:click={() => open(dir)}>
          <div class="font-medium">{dir.name}</div>
        </button>
        <button class="rounded-md bg-rose-600/90 hover:bg-rose-600 active:bg-rose-700 px-3 py-2 text-xs font-medium transition text-white" on:click={() => askDelete(dir.id)}>
          {$t('delete')}
        </button>
      </div>
    {/each}
